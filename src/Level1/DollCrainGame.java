...
//		상태는 아래 그림과 같으며 바구니에 담는 과정에서 터트려져 사라진 인형은 4개 입니다.
		...
		while(true){
			for(int j = 0 ; j<stk.size(); j++){
				...
				if(stk.get(j)==stk.get(j+1)){
					System.out.println(j+" "+stk.get(j));
					System.out.println((j+1)+" "+stk.get(j+1));
					stk.remove(j);
					stk.remove(j);
					cnt+=2;
					allChk =false;
